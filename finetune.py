...
def finetune(config, writer, device_idxs=[0]):
    ...
    data_loader = DataLoader(dataset, batch_size=config['batchsize'], num_workers=0, pin_memory=True, shuffle=True, drop_last=False)
    ...
    with torch.no_grad():
        try:
            ...
            for i, data in iterator:
                ...
                cv2.imwrite('full_class_bodydebug_nearest.png', class_body_full)
                cv2.imwrite('full_class_bodydebugsmall_nearest.png', class_body_resize)
                print ('saving')
                exit()
